                    ...
                    using (XmlReader dr = cmd.ExecuteXmlReader())
                    {
                        while (dr.Read())
                        {
                            ...
                            if (doc.Element("DETALLE_VENTA") != null)
                            {
                                ...
                                rptDetalleVenta.oBoleterias = (from dato in doc.Element("DETALLE_VENTA").Elements("DETALLE_BOLETERIAS")
                                ...
                                rptDetalleVenta.oListaDetalleVenta = (from concierto in doc.Element("DETALLE_VENTA").Element("DETALLE_CONCIERTO").Elements("CONCIERTO")
                                                                      select new DetalleVenta()
                                                                      {
                                                                          Cantidad = int.Parse(concierto.Element("Cantidad").Value),
                                                                          NombreConcierto = concierto.Element("NombreConcierto").Value,
                                                                          PrecioUnidad = float.Parse(concierto.Element("PrecioUnidad").Value, NuevaCultura),
                                                                          ImporteTotal = float.Parse(concierto.Element("ImporteTotal").Value, NuevaCultura)
                                                                      }).ToList();
                            }
                            else
                            {
